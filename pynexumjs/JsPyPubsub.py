...
def publish(topic: str, suppress: bool=False) -> typing.Callable:
    ...
    ----------
    // You need to load 'manager.js' and 'JsPyPubsub.js' in advance.
    ...
    def inner(message) -> bool:
        ...
        if topic in _topic_manager:
            ...
            WebSocketManager.reservecast(send_dic, _topic_manager[topic])
        return(True)
    return inner
    ...
    try:
        if topic in _topic_manager:
            if suppress is False:
                WebSocketManager.multicast(send_dic, _topic_manager[topic])
            else:
                WebSocketManager.multicast(send_dic,
                    _topic_manager[topic]-{socket_id})
    except:
        pass

WebSocketManager.add_protocol('sub_call', _sub_from_js)
WebSocketManager.add_protocol('unsub_call', _unsub_from_js)
WebSocketManager.add_protocol('pub_call', _pub_from_js)
...
WebSocketManager.add_socket_event(_socket_event)
